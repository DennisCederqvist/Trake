...
export class MultiplayerController {
	...
	frame(_delta) {
		...
		this.game.mySnake.segments.forEach(seg => {
			data.list.push({ x: seg.x, y: seg.y });
		});
	...
	onGameMessage(fromClientId, data) {
		...
		switch (data.type) {
			...
			case "segments":
				console.log("Segments from", fromClientId, data.list);
				break;
	...
	startMultiplayerMatchHost() {
		...
		this.game.onFrame.push(this.frame.bind(this));
